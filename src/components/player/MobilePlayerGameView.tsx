...
export default function MobilePlayerGameView({
  ...
  // Card dimensions for carousel effect
  ...
  // Handle scroll events to provide real-time feedback and detect scroll end
  const handleScroll = () => {
    if (!scrollViewRef.current) return;
    ...
    // Momentum scrolling: detect when scrolling has stopped, then snap.
    if (scrollEndTimer.current) {
      clearTimeout(scrollEndTimer.current);
    }
    
    scrollEndTimer.current = setTimeout(() => {
      // Only snap if the user is not actively dragging
      if (!isDragging) {
        snapToNearestGap();
    ...
    }, 150); // A 150ms delay is standard for detecting scroll end
  };
  ...
  // Handle touch end to stop dragging state
  const handleTouchEnd = () => {
    setIsDragging(false);
    // The snap is now handled by the onScroll logic to allow momentum.
  };
  ...
  useEffect(() => {
    if (isMyTurn && !gameEnded) {
      ...
      if (scrollViewRef.current) {
        // Scroll to the beginning (position 0) on new turn
        scrollViewRef.current.scrollTo({ 
          left: 0,
          behavior: 'smooth' 
        });
  ...
  return (
    // Set explicit height and hide vertical overflow to fit screen without scrolling
    <div className="h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-indigo-900 flex flex-col relative overflow-y-hidden overflow-x-hidden">
      {/* Background Effects */}
      <div className="absolute inset-0 overflow-hidden">
        ...
        <div className="relative z-10 flex-1 flex items-center justify-center px-4 py-8">
          <div className="text-center space-y-8 max-w-xs">
            <div className="relative">
              ...
              <div className={`relative w-40 h-40 mx-auto transition-all duration-500 ${isPlaying ? 'animate-spin' : 'hover:scale-110'}`}>
          ...
          <div className="bg-gradient-to-r from-white/20 to-white/15 backdrop-blur-xl rounded-3xl p-6 border border-white/30 shadow-2xl">
            ...
            <div className="relative mb-6 h-10">
              ...
              <div className="absolute inset-0 flex justify-between items-center">
                {Array.from({ length: timelineCards.length + 1 }, (_, i) => (
                  <div key={i} className={`w-2 h-6 rounded-full transition-all duration-500 ${snappedPosition === i ? "bg-gradient-to-t from-green-400 to-green-300 shadow-xl scale-150 animate-pulse" : "bg-white/60 hover:bg-white/80"}`} />
                ))}
              </div>
            ...
            >
              <div 
                ...
                style={{ 
                  // Dynamic padding ensures first/last items can be centered
              ...
              >
                {timelineCards.map((song, index) => (
                  <React.Fragment key={song.id || index}>
                    {/* Gap Indicator */}
                    <div 
                      className={`flex-shrink-0 rounded-2xl border-3 border-dashed transition-all duration-500 flex items-center justify-center ${snappedPosition === index ? "border-green-400 bg-gradient-to-br from-green-400/30 to-green-300/20 scale-110 shadow-2xl animate-pulse" : "border-white/50 bg-white/10 hover:bg-white/15 hover:border-white/70"}`}
                      style={{ width: `${GAP_WIDTH}px`, height: `${CARD_HEIGHT}px` }}
                    >
                      <div className={`w-4 h-4 rounded-full transition-all duration-500 ${snappedPosition === index ? "bg-gradient-to-br from-green-400 to-green-300 shadow-xl animate-ping" : "bg-white/70 hover:bg-white/90"}`} />
                    </div>
                    ...
                    >
                      <div className={`absolute inset-0 bg-gradient-to-br from-slate-600 via-slate-700 to-slate-900 rounded-2xl border-2 border-white/40 flex flex-col items-center justify-center shadow-2xl transition-all duration-500 group-hover:scale-105 group-hover:rotate-y-12 group-hover:shadow-3xl ${playingPreviewId === song.id ? 'animate-pulse border-yellow-400 bg-gradient-to-br from-yellow-900 via-yellow-800 to-yellow-700' : ''}`}>
                        ...
                        <div className="relative z-10 text-center p-3">
                          <div className="text-white font-black text-2xl mb-2 drop-shadow-lg">{song.release_year}</div>
                        ...
                        {song.preview_url && (
                          <div className="absolute bottom-2 right-2 w-8 h-8 bg-black/70 rounded-full flex items-center justify-center backdrop-blur-sm border border-white/30 group-hover:scale-110 transition-transform">
                            {playingPreviewId === song.id ? <Pause className="w-4 h-4 text-white animate-pulse" /> : <Play className="w-4 h-4 text-white ml-0.5" />}
                          </div>
                        )}
                      </div>
                    </div>
                ...
                {/* Final Gap */}
                <div 
                  className={`flex-shrink-0 rounded-2xl border-3 border-dashed transition-all duration-500 flex items-center justify-center ${snappedPosition === timelineCards.length ? "border-green-400 bg-gradient-to-br from-green-400/30 to-green-300/20 scale-110 shadow-2xl animate-pulse" : "border-white/50 bg-white/10 hover:bg-white/15 hover:border-white/70"}`}
                  style={{ width: `${GAP_WIDTH}px`, height: `${CARD_HEIGHT}px` }}
                >
                  <div className={`w-4 h-4 rounded-full transition-all duration-500 ${snappedPosition === timelineCards.length ? "bg-gradient-to-br from-green-400 to-green-300 shadow-xl animate-ping" : "bg-white/70 hover:bg-white/90"}`} />
                </div>
              </div>
            </div>
      ...
      {isMyTurn && !gameEnded && (
        <div className="relative z-10 px-4 pb-8">
          <Button
            ...
            className={`w-full h-18 text-white font-black text-xl rounded-3xl border-0 shadow-2xl transition-all duration-500 relative overflow-hidden ${hasConfirmed || isSubmitting ? "bg-gradient-to-r from-gray-600 to-gray-700" : "bg-gradient-to-r from-green-500 via-emerald-500 to-green-600 hover:from-green-600 hover:via-emerald-600 hover:to-green-700 hover:scale-105 active:scale-95 hover:shadow-3xl"}`}
          >
